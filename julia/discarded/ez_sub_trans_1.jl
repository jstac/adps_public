...
The base model 
    ...
    B(w, e, s, v) = { r(s, w, e)^α + β [Σ_e' v^γ(s, e') φ(e')]^{α/γ} }^{1/γ}

is transformed to 

    B(w, e, s, v) = { r(s, w, e)^α + β [ v(s, e') φ(e')]^{1/θ} }^θ

The subordinate ADP has policy operator

    (B_σ h)(w) = Σ_e { r(w, σ(w), e)^α + β h(σ(w))^(1/θ) }^θ φ(e)
    
and an optimal policy is found by solving

    σ(w, e) = argmin_s { r(w, s, e)^α + β h(s)^(1/θ) }
...
function B(i, k, h::Vector, model)
    ...
    value = Inf
    function f(e)
        return ((w - s + e)^α + β * h[k]^(1/θ))^θ
    end
    if s <= w
        value = @views dot(f.(e_grid), φ)
    end
    return value
end
...
v_star, σ_star = optimistic_policy_iteration(v_init, B, model)

println("Solving modified model.")

h_init = ones(length(model.w_grid))
h_star, _ = optimistic_policy_iteration(h_init, B, model)
